    ...
    {etapas.map(f => (
      <div key={f.etapa} className="card" style={{marginBottom:10}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          {showRol && <Tag bg="#E9EEF5" color="var(--tx)">{f.rol}</Tag>}
          <Tag>{f.act}</Tag>
        ...
        {f.guia && <div className="note" style={{background:"rgba(31,78,140,.08)",border:"1px solid var(--bd)",color:"var(--tx)",marginTop:8,fontSize:12,lineHeight:1.5}}>
        ...
        {f.penDesc && <div className="note" style={{background:"rgba(120,53,15,.10)",border:"1px solid #78350f",color:"#7A4A0A",marginTop:10}}>Si falla: {f.penDesc}</div>}
      </div>
    ))}
  </>;
}
