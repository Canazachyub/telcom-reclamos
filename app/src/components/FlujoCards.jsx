    ...
    {etapas.map(f => (
      <div key={f.etapa} className="card" style={{marginBottom:10}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          {showRol && <Tag bg="var(--card2)" color="var(--tx)">{f.rol}</Tag>}
          <Tag>{f.act}</Tag>
          <Tag>plazo: {f.plazo}</Tag>
          {f.pen!=="—" && <Tag bg="var(--amber)" color="var(--ink)">penalidad {f.pen}</Tag>}
        </div>
        {f.desc && <div style={{fontSize:12.5,color:"var(--tx)",margin:"9px 0",lineHeight:1.55}}>{f.desc}</div>}
        ...
        {f.pasos.map((p,k)=><div className="chk" key={k}><span style={{color:"var(--mut)"}}>○</span> {p}</div>)}
        ...
        {f.guia && <div className="note" style={{background:"var(--tint-acc-bg)",border:"1px solid var(--bd)",color:"var(--tx)",marginTop:8,fontSize:12,lineHeight:1.5}}>
          <div><b style={{color:"var(--mut)"}}>Formato admitido:</b> {f.guia.formatos} <span style={{color:"var(--mut)"}}>— PDF de preferencia</span></div>
          <div style={{marginTop:3}}><b style={{color:"var(--mut)"}}>Qué se espera:</b> {f.guia.espera}</div>
        </div>}
        {f.penDesc && <div className="note" style={{background:"var(--tint-amber-bg)",border:"1px solid var(--tint-amber-bd)",color:"var(--tint-amber-tx)",marginTop:10}}>Si falla: {f.penDesc}</div>}
      </div>
    ))}
  </>;
}
